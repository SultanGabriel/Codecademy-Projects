...
function downvoteComment(url, request) {
    ...
    if (user && savedComment && !savedComment.downvotedBy.includes(username)) {
        if (savedComment.upvotedBy.includes(username)) {
            const i = savedComment.upvotedBy.indexOf(username);
            savedComment.upvotedBy.splice(i, 1);
        }
...
function upvoteComment(url, request) {
    ...
    if (user && savedComment && !savedComment.upvotedBy.includes(username)) {
        if (savedComment.downvotedBy.includes(username)) {
            const i = savedComment.downvotedBy.indexOf(username);
            savedComment.downvotedBy.splice(i, 1);
        }
